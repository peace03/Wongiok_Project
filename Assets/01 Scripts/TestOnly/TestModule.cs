...
public class TestModule : MonoBehaviour
{
    ...
    [SerializeField] private string titleSceneName = "TestUI";
    ...
    {
        currentExp = Mathf.Min(currentExp + expGainAmount, requiredExp);
    ...
    private void ShowLevelUp()
    {
        ...
        EventBus<UISetLevelUpOptionsEvent>.Publish(
            new UISetLevelUpOptionsEvent(CreateLevelUpOptionsFromRealSkills()));
    ...
    private void PublishPlayerState()
    {
        EventBus<UISetPlayerLevelEvent>.Publish(
            new UISetPlayerLevelEvent(playerLevel));
        ...
        EventBus<UISetPlayerLifeEvent>.Publish(
            new UISetPlayerLifeEvent(currentLife, maxLife));
    ...
    private void HandleGameOverRestartChapterRequested(UIGameOverRestartChapterRequestedEvent eventData)
    {
        ...
        PrototypeProgressSnapshot chapterStart = PrototypeGameSession.GetChapterStart();

        ApplyPrototypeSnapshot(chapterStart);
    }

    private void HandleGameOverLoadCheckPointRequested(UIGameOverLoadCheckpointRequestedEvent eventData)
    {
        if (!canLoadCheckPoint || !PrototypeGameSession.TryGetCheckpoint(out PrototypeProgressSnapshot checkpoint))
        {
        ...
        ApplyPrototypeSnapshot(checkpoint);
    }
